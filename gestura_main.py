...
class GesturaRuntime:
    ...
    def process_gesture(self, gesture):
        ...
        elif self.mode == "CONDITIONAL_TYPE_MODE":
            self.handle_conditional_type_mode(gesture)
    ...
    def handle_normal_mode(self, gesture):
        if gesture == "SET":
            ...
            self.mode = "CONDITIONAL_TYPE_MODE"
            self.message = "CONDITIONAL: ONE=IF, TWO=ELIF, THREE=ELSE"
    ...
    def handle_conditional_type_mode(self, gesture):
        if gesture in CONDITIONAL_TYPE_MAP:
            cond_type = CONDITIONAL_TYPE_MAP[gesture]
            ...
            self.condition_context = cond_type
            self.condition_parts = []
            self.mode = "CONDITION_VALUE_TYPE_MODE"
            self.message = f"{cond_type} MODE: Choose first value. ONE=variable, TWO=number"
    ...
    def finish_condition_header(self):
        ...
        elif self.condition_context in ["IF", "ELIF"]:
            command = {
                "type": f"{self.condition_context}_START",
                "condition": condition,
            }

            self.program_commands.append(command)

            if self.condition_context == "ELIF":
                indent = max(0, indent - 1)

            gestura_line = f"{self.condition_context}({condition}) OK"
    ...
    def current_indent(self):
        ...
        for command in self.program_commands:
            if command["type"] in ["LOOP_START", "IF_START", "ELIF_START", "ELSE"]:
                indent += 1
    ...
    def run_block(self, start_index, end_index, max_loop_count=100):
        ...
        while i < end_index:
            ...
            if command["type"] == "SET":
                ...
                i = self.run_if_chain(i, max_loop_count)

            elif command["type"] in ["ELIF_START", "ELSE"]:
                return
    ...
    def run_if_chain(self, if_index, max_loop_count):
        end_index = self.find_matching_end(if_index)
        ...
        while i < end_index:
            ...
            if command_type in ["LOOP_START", "IF_START"]:
                ...
                depth = max(0, depth - 1)
            elif depth == 0 and command_type in ["ELIF_START", "ELSE"]:
                chain_parts.append((current_start, i))
                current_start = i

            i += 1

        chain_parts.append((current_start, end_index))

        for start, stop in chain_parts:
            command = self.program_commands[start]

            if command["type"] == "IF_START":
                if self.evaluate_condition(command["condition"]):
                    self.run_block(start + 1, stop, max_loop_count)
                    break

            elif command["type"] == "ELIF_START":
                if self.evaluate_condition(command["condition"]):
                    self.run_block(start + 1, stop, max_loop_count)
                    break
