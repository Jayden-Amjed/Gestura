...
class GesturaRuntime:
    def __init__(self):
        ...
        self.message = "NORMAL MODE: Show SET, PRINT, LOOP, END, or EXECUTE"
    ...
    def process_gesture(self, gesture):
        ...
        elif self.mode == "LOOP_VALUE_TYPE_MODE":
            self.handle_loop_value_type_mode(gesture)

        elif self.mode == "LOOP_VARIABLE_MODE":
            self.handle_loop_variable_mode(gesture)

        elif self.mode == "LOOP_NUMBER_MODE":
            self.handle_loop_number_mode(gesture)

        elif self.mode == "LOOP_OPERATOR_TYPE_MODE":
            self.handle_loop_operator_type_mode(gesture)
    ...
    def handle_loop_mode(self, gesture):
        if gesture in LOOP_MAP:
            ...
            self.mode = "LOOP_VALUE_TYPE_MODE"
    ...
    def handle_loop_value_type_mode(self, gesture):
        if gesture == "ONE":
            self.mode = "LOOP_VARIABLE_MODE"
            self.message = "LOOP VARIABLE MODE: Pick variable ONE-FIVE"

        elif gesture == "TWO":
            self.mode = "LOOP_NUMBER_MODE"
            self.number_total = 0
            self.message = "LOOP NUMBER MODE: Add number gestures. OK saves number."

        elif gesture == "OK":
            self.finish_loop_header()

        else:
            self.message = "Choose first value first: ONE=variable or TWO=number"

    def handle_loop_variable_mode(self, gesture):
        if gesture in VARIABLE_MAP:
            variable_name = VARIABLE_MAP[gesture]
            self.condition_parts.append(variable_name)
            self.mode = "LOOP_OPERATOR_TYPE_MODE"
            ...
            self.message = "LOOP VARIABLE MODE: Pick ONE-FIVE"

    def handle_loop_number_mode(self, gesture):
        if gesture in NUMBER_MAP:
            self.number_total += NUMBER_MAP[gesture]
            self.message = f"LOOP NUMBER MODE: Current number = {self.number_total}. OK saves."
            ...
            self.message = "LOOP NUMBER MODE: Show ONE-FIVE or OK"

    def handle_loop_operator_type_mode(self, gesture):
    ...
    def finish_loop_header(self):
        condition = " ".join(self.condition_parts)

        if not self.loop_type or not condition:
    ...
    def find_matching_end(self, loop_start_index):
        depth = 0

        for i in range(loop_start_index + 1, len(self.program_commands)):
            command_type = self.program_commands[i]["type"]

            if command_type == "LOOP_START":
                depth += 1
    ...
    def get_status_lines(self):
        ...
        if self.mode in ["NUMBER_MODE", "LOOP_NUMBER_MODE"]:
        ...
        if self.mode.startswith("LOOP") or self.mode in [
            "MATH_OPERATOR_MODE",
            "EQUALITY_OPERATOR_MODE",
            "COMPARISON_OPERATOR_MODE",
        ]:
            lines.append(f"Loop type: {self.loop_type}")
